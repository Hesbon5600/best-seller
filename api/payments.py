...
def handle_error(function):
    def wrapper(*args, **kwargs):
        try:
            ...
            return PaymentError(message=err.get('message'),
            ...
            return PaymentError(
                message="Too many requests made to the API too quickly", status=e.http_status)
        except stripe.error.InvalidRequestError as e:
            # Invalid parameters were supplied to Stripe's API
            logger.error(e.json_body['error']['message'])
            return PaymentError(
                message=e.json_body['error']['message'], status=e.http_status, _type=e.json_body['error']['type'])
        except stripe.error.AuthenticationError as e:
            # Authentication with Stripe's API failed
            # (maybe you changed API keys recently)
            logger.error('Something went wrong!')
            return PaymentError(message="Invalid API Key", status=e.http_status)
        except stripe.error.APIConnectionError as e:
            # Network communication with Stripe failed
            logger.error('Network communication with Stripe failed')
            return PaymentError(
                message="Network communication with Stripe failed", status=e.http_status)
        except stripe.error.StripeError as e:
            # Display a very generic error to the user, and maybe send
            # yourself an email
            logger.error(e.json_body['error']['message'])
            return PaymentError(message=e.json_body['error']['message'], status=e.http_status,
                                _type=e.json_body['error']['type'])
        except Exception as e:
            # Something else happened, completely unrelated to Stripe
            logger.error('Something went wrong!')
            return PaymentError(message="There is something wrong", status=500)
